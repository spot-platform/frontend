...
function toProfileFormState(
    profile: NonNullable<
        NonNullable<ReturnType<typeof useMyProfile>['data']>['data']
    >,
): ProfileFormState {
    return {
...
export function MySettingsPageClient() {
    ...
    const initialProfileForm = useMemo(
        () => (profile ? toProfileFormState(profile) : EMPTY_PROFILE_FORM),
        [profile],
    );
    const profileForm = profileFormDraft ?? initialProfileForm;
